...
- **Tabs** (`.tabs` / `.tab`, ARIA `role="tablist"`) — two tabs: **Images** (`#tabbtn-images` → `#tab-images`) and **Nested emphasis** (`#tabbtn-emphasis` → `#tab-emphasis`). Images is the first/default tab. `selectTab(idx)` toggles the `active` class and `aria-selected`, and shows/hides the matching `.tab-panel`. Styling: thin **1px** borders with rounded top corners; the panels sit in a `.tab-body` wrapper with a 1px top border. Layering is done with `z-index`, not just color — the active tab (`z-index: 3`, `top: 0`) sits above the `.tab-body` (`z-index: 2`), while the inactive tab (`z-index: 1`, `top: 3px`) sits *behind* both, so it reads as tucked under the active tab. Tabs overlap horizontally (`.tab + .tab { margin-left: -10px }`, with extra `padding-left` for text clearance) to reinforce the layered look. If you change border thickness, keep `.tab`'s `margin-bottom` and `.tab-body`'s `border-top` in sync so the active tab's base still merges cleanly into the panel.
- **Stable height (do not remove).** `.tab-panel` has a fixed `min-height` (currently `360px`) sized to comfortably fit the *taller* panel (nested emphasis, 3 options) plus a little breathing room. Without it, switching to the shorter Images panel would shrink the modal and slide the tab bar out from under the cursor. If you add options to either panel, re-measure the taller panel's natural `scrollHeight` and raise this `min-height` so both tabs keep the modal the same size.
