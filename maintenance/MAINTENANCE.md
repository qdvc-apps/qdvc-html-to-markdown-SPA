...
`renderBlockquote(n)` is shared by both the `BLOCKQUOTE` tag case and callouts. It first tries `block(n, "")` (for callouts containing block children like `<p>`, headings, or lists); if that yields nothing — the common `<p class="callout">inline text</p>` case — it falls back to `inline(n)`. The result is prefixed line-by-line with `> ` (blank lines become a bare `>`), so multi-paragraph and block content inside a callout quote correctly.
...
- **Header + tabs are unified.** `.modal-head` has no bottom border, so the "Settings" title and the tab bar read as one continuous header with no dividing line between them.
- **Tabs** (`.tabs` / `.tab`, ARIA `role="tablist"`) — three tabs: **Images** (`#tabbtn-images` → `#tab-images`), **Nested emphasis** (`#tabbtn-emphasis` → `#tab-emphasis`), and **Callouts** (`#tabbtn-callouts` → `#tab-callouts`). Images is the first/default tab. `selectTab(idx)` toggles the `active` class and `aria-selected`, and shows/hides the matching `.tab-panel`; the `tabButtons`/`tabPanels` arrays in the wiring must list all three in order. Styling: thin **1px** borders with rounded top corners; the panels sit in a `.tab-body` wrapper with a 1px top border and a **solid `--panel` background** (so the inactive tabs, which poke down behind the panel at `top: 3px`, are fully hidden by the panel area — do not remove this fill). Layering is done with `z-index` — the active tab (`z-index: 3`, `top: 0`) sits above the `.tab-body` (`z-index: 2`), while inactive tabs (`z-index: 1`, `top: 3px`) sit *behind* both, tucked under the active tab. The active tab also has **`border-bottom: 1px solid var(--panel)`** — a bottom border matching its own background — so it covers the `.tab-body`'s top border directly beneath it and shows *no line under the active tab*. Tabs overlap horizontally (`.tab + .tab { margin-left: -10px }`, with extra `padding-left` for text clearance) to reinforce the layered look. If you change border thickness, keep `.tab`'s `margin-bottom`, the active tab's `border-bottom`, and `.tab-body`'s `border-top` in sync so the active tab's base still merges cleanly into the panel.
- **Stable height (do not remove).** `.tab-panel` has a fixed `min-height` (currently `360px`) sized to comfortably fit the *tallest* panel (nested emphasis, ~327px natural) plus a little breathing room; the Images (~217px) and Callouts (~256px) panels fit within it. Without it, switching to a shorter panel would shrink the modal and slide the tab bar out from under the cursor. If you add options to any panel, re-measure the tallest panel's natural `scrollHeight` and raise this `min-height` so all tabs keep the modal the same size.
...
- **A line appears under the active tab, or an inactive tab shows through** → the active tab needs `border-bottom: 1px solid var(--panel)` and `.tab-body` needs a solid `--panel` background (§6.5).
