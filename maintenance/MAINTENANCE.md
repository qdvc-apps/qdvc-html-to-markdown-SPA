...
4. **`<script>`** — one IIFE (`(function () { "use strict"; ... })();`) containing all logic. At the top of the IIFE is the `SETTINGS` object and the `emphasize` helper (see §3.5). Below that, the converter, the URL scanner (`extractUrls` / `buildLinksReport`, see §4.5), the preview renderer, and three parts separated by comment banners:
   ...
   - the **DOM wiring** (`// --- wiring ---`), which also handles opening/closing the modal, tab switching, reacting to settings changes, and the two independent output toggles (see §4.5).
...
- **`block(node, indent)` / `renderBlock(n, indent)`** — produce block-level Markdown, joining children with blank lines. Before the main switch, `renderBlock` short-circuits callout `<div>`/`<p>` elements to a blockquote when enabled (see §3.6). It then handles headings, `P`, `HR`, `BLOCKQUOTE`, `PRE`, `UL`/`OL`, `DL`, `TABLE`, `FIGURE`, and strips `SCRIPT`/`STYLE`/`NOSCRIPT`/`TEMPLATE`. Unknown tags are classified via `hasBlockChild` / the `BLOCK` set and recursed into as either a block container or an inline run.
...
| `detectCallouts` | `true` / `false` | `true` | When true, a `<div>`/`<p>` whose class contains "callout" becomes a blockquote (see §3.6). When false, such tags convert normally. |
...
When `SETTINGS.detectCallouts` is true, `renderBlock` short-circuits *before* the main `switch`: if the element is a `<div>` or `<p>` and `isCallout(n)` is true, the whole element is rendered as a blockquote via `renderBlockquote(n)`. `isCallout` splits the element's `class` attribute on whitespace and returns true if any class matches `/callout/i` — a substring match, so `callout`, `callout-warning`, and `note callout-info` all qualify (per the spec, "any class with the word callout in its name").

`renderBlockquote(n)` is shared by both the `BLOCKQUOTE` tag case and callouts, and it chooses its strategy by `hasBlockChild(n)`. **This choice matters and must not be simplified back to "always `block()`".** If the element has real block children (`<p>`, headings, lists, …) it uses `block(n, "")`, which quotes each child as its own paragraph. Otherwise — the common case of a callout/quote holding inline content like `<b>`, `<a>`, `<code>`, and text — it renders the whole thing as a single run via `inline(n)`. Feeding inline children to `block()` would be wrong twice over: each inline element would hit `renderBlock`'s `default` case and be *unwrapped* (losing its `**`/link/`` ` `` markers), and `block()` would join them with blank lines, inserting a spurious break after every tag. (That double bug — stripped formatting plus per-tag line breaks — is exactly what an earlier version did; the `hasBlockChild` split fixes it.) Either way the result is prefixed line-by-line with `> ` (blank lines become a bare `>`), so multi-paragraph and block content still quote correctly.

Because detection happens before the `switch`, turning the setting off makes callout tags fall through to their normal handling (`<p>` → paragraph, `<div>` → unwrapped container via the `default` branch). Note the Markdown has no way to mark a blockquote as a *specific* callout type (warning/info/success), which is why the preview renders all blockquotes with a neutral grey rule rather than a colour that would imply one type (see §5).
...
## 4.5. URL scanner and the two output toggles
...
**Two independent output toggles.** The output view is the product of two *orthogonal* toggles, each held in its own variable — they deliberately do **not** interfere:

- `content` — `"doc"` (convert the whole snippet, default) or `"links"` (the URL report). The **Links** button flips it and its label mirrors Preview/Source: it reads **Links** while showing the doc, **Doc** while showing the report.
- `render` — `"source"` (raw Markdown in the `<pre>`, default) or `"preview"` (rendered HTML). The **Preview** button flips it; its label reads **Preview** then **Source**.

All four combinations are valid (doc/source, doc/preview, links/source, links/preview). `run()` chooses the Markdown by `content` (`buildLinksReport(src)` vs `convert(src)`) and refreshes the preview pane when `render === "preview"`. `applyView()` syncs both buttons' labels and `.done` state and the source/preview element visibility, then calls `run()`. Because every input/settings/clear/sample handler calls `run()`, whichever combination is active stays live as the user types or changes settings. To add another *content* mode, widen the `content` values and the `run()` branch; to add another *render* mode, widen `render` and `applyView()`. Keep the two axes separate — collapsing them back into one shared state variable is what the "don't interfere" requirement forbids.
...
| `--accent-soft` | `#6a8a5f` | soft green accent (option borders) |
| `--amber` | `#b3541e` | crimson/amber highlight, links, favicon arrow |
| `--mute` | `#7b7365` | secondary text |

The preview blockquote rule uses a neutral grey (`#9a917f`), **not** a green accent, and this is deliberate: blockquotes can now represent callouts of any type (warning/info/success), so a green ("success") colour would be misleading. Keep it grey/neutral. The favicon uses `--paper`, `--amber`, and `--ink` values directly. If you change the palette, regenerate the favicon SVG to match and re-embed it (see §6).
...
The modal markup is `#settings-overlay` (a fixed full-screen `.overlay` containing a `.modal`), hidden via the `hidden` attribute by default. It is opened by the header **Settings** gear (`#open-settings`). It uses a **master-detail** layout (a horizontal tab bar was tried earlier and abandoned as fiddly). Structure:

- **Header.** `.modal-head` (title + close ✕) with a 1px bottom border, above the body.
- **Master-detail body.** `.modal-body` is a flex row: a fixed-width left `.side-nav` (`role="tablist"`) holds the section buttons stacked vertically; the right `.detail` holds the panels. The section buttons still use the class `.tab` and the panels `.tab-panel`, and the JS is unchanged — `selectTab(idx)` toggles the `active` class + `aria-selected` on each button and shows/hides the matching panel. The three sections are **Images** (`#tabbtn-images` → `#tab-images`), **Nested emphasis** (`#tabbtn-emphasis` → `#tab-emphasis`), and **Callouts** (`#tabbtn-callouts` → `#tab-callouts`); Images is the default. The `tabButtons`/`tabPanels` arrays in the wiring must list all three in order.
- **Active-item styling.** A `.tab` is a left-aligned button with a transparent `border-left`; the active one gets a green (`--accent`) left border, `--panel` background, and accent text. No overlapping/z-index tricks are needed anymore — that complexity went away with the tab bar.
- **Stable height (do not remove).** `.tab-panel` has a fixed `min-height` (currently `370px`) sized to the *tallest* panel (nested emphasis, ~363px natural); Images (~217px) and Callouts (~275px) fit within it. Without it the modal would resize as you switch sections. If you add options to any panel, re-measure the tallest panel's natural `scrollHeight` and raise this `min-height`. (Height stability matters less than it did with a top tab bar — the nav is on the left, so a resize wouldn't move the item you clicked — but keeping it steady still looks better.)
- **Responsive.** Under 760px the `.modal-body` switches to `flex-direction: column` and `.side-nav` becomes a horizontal, scrollable row on top (`.tab` borders flip from left to bottom), so it stays usable on narrow screens.
- **Options** are radio groups: `name="images"` (`discard` default / `keep`), `name="emphasis"` (`reverse` / `stack` / `discard` default), and `name="callouts"` (`detect` default / `off`). Their `change` handlers set the corresponding `SETTINGS` key and call `run()` for a live re-convert.
...
The defaults in the HTML (`checked` attributes) must stay in sync with the defaults in the `SETTINGS` object: images `discard`, emphasis `discard`, callouts `detect`. If you add a setting, add it to `SETTINGS`, add a tab/panel or option here (and to the `tabButtons`/`tabPanels` arrays if it's a new tab), wire a `change` handler that updates `SETTINGS` and calls `run()`, and document it in §3.5.

## 7. Testing changes

There is no test harness in the repo, but the converter functions are pure and can be exercised in Node with `jsdom` providing `DOMParser` and `document`. A quick approach: extract the script text, slice off everything from the `// --- wiring ---` banner onward (that part needs real DOM elements), `eval` the pure-function portion, and call `convert(...)` on sample HTML. Toggle behavior by setting `SETTINGS.discardImages` / `SETTINGS.nestedEmphasis` / `SETTINGS.detectCallouts` between calls. `extractUrls` / `buildLinksReport` are pure too — test them with tag soup containing duplicate URLs, mixed schemes (`http`, `https`, `ftp`, `smb`), URLs in attributes vs plain text, and trailing punctuation, expecting a de-duplicated, alphabetically sorted, backtick-wrapped report. Always test at least: a citation/DOI-heavy paragraph (guards against escaping regressions), nested lists, a table, a fenced code block, a paragraph beginning with `1. ` or `- ` (guards `escLineStart`), **adjacent emphasized runs like `<i>a </i><i>b</i>` (guards the boundary-whitespace fix — expect `*a* *b*`, never `*a**b*`), same-kind nested emphasis under all three modes, and callout `<div>`/`<p>` (both inline-only and block-child variants) with detection on and off**. Piping the output through a real CommonMark parser (`commonmark` on npm) is the best way to confirm the Markdown actually renders as intended.

For the modal, output toggles, and settings wiring, load the whole file in `jsdom` with `runScripts: "dangerously"`, then drive it via `.click()` and dispatched `input`/`change` events — this catches wiring regressions the pure-function tests can't. Worth asserting: the **Links** and **Preview** toggles are independent — exercise all four combinations (doc/source, doc/preview, links/source, links/preview) and confirm the button labels ("Links"↔"Doc", "Preview"↔"Source") and `.done` states match, and that editing input updates whichever combination is active. For the settings modal's stable-height guarantee, a headless browser (Playwright) can compare `.modal` bounding-box heights across all three sections — they should be equal.

Manual smoke test before shipping: open the file, click **Sample**, confirm the output pane looks right, toggle **Links** and **Preview**, click **Copy**, then open **Settings**, switch between the sidebar sections (confirming the modal doesn't resize), and flip each option while watching the output update. Check a callout with inline formatting (bold/link/code) quotes on one line with markup intact.
...
- **Callout not detected / wrongly detected** → `isCallout` matches `/callout/i` per class token on `<div>`/`<p>`; the short-circuit is at the top of `renderBlock` and gated on `SETTINGS.detectCallouts` (§3.6).
- **Callout/blockquote loses inline formatting or gains breaks between tags** → `renderBlockquote` must use `inline(n)` (not `block()`) when the element has no block children; see §3.6.
- **Settings modal resizes when switching sections** → the `.tab-panel` `min-height` no longer covers the tallest panel; re-measure and raise it (§6.5).
...
- **Links/Doc and Preview/Source interfere with each other** → they must stay as two separate variables (`content`, `render`); don't merge them into one shared state (§4.5).
- **Adding another output mode** → widen `content` (for a new document type) or `render` (for a new display mode) plus the matching branch in `run()`/`applyView()`; keep the two axes independent (§4.5).
